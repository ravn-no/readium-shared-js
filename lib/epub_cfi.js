...
EPUBcfi.CFIInstructions = {
	...
	inferTargetTextNode_OLD_WITH_BUG : function (CFIStepValue, $currNode, classBlacklist, elementBlacklist, idBlacklist) {
    ...
    // New implementation with bugfix
	inferTargetTextNode : function (CFIStepValue, $currNode, classBlacklist, elementBlacklist, idBlacklist) {
		var $elementsWithoutMarkers = this.applyBlacklist($currNode.contents(), classBlacklist, elementBlacklist, idBlacklist);;
        CFIStepValue = parseInt(CFIStepValue, 10);

        var CFIIndex = 1;
        var targetTextNodeList = [];

		$elementsWithoutMarkers.each(function () {
            if (this.nodeType === Node.TEXT_NODE) {
                if (CFIIndex === CFIStepValue) {
                    targetTextNodeList.push(this);
                }
            } else if (targetTextNodeList.length) {
                // We have collected all the text-nodes with the right CFIindex, and have now reached a non-textnode.
                return false; // break
            } else {
                // We have still not reached a text-node with the right CFI index
                CFIIndex += 2;
            }
        });

		if (targetTextNodeList.length === 0) {
            // NOTE: the arguments differ from the original code. But this seems to make more sense...
			throw EPUBcfi.OutOfRangeError(CFIStepValue, CFIIndex, "Index out of range");
		}
		return $(targetTextNodeList);
...
EPUBcfi.CFIAssertionError = function (expectedAssertion, targetElementAssertion, message) {
    ...
    createCFITextNodeStep_OLD_WITH_BUG : function ($startTextNode, characterOffset, classBlacklist, elementBlacklist, idBlacklist) {
    ...
    createCFITextNodeStep : function ($startTextNode, characterOffset, classBlacklist, elementBlacklist, idBlacklist) {
        ...
        var $contentsExcludingMarkers = EPUBcfi.CFIInstructions.applyBlacklist($parentNode.contents(), classBlacklist, elementBlacklist, idBlacklist);
        var CFIIndex = 1; // The first potention CFI text position starts at 1
        var CFICharacterOffset = 0;
        $.each($contentsExcludingMarkers, function () {
            if (this.nodeType === Node.TEXT_NODE) {
                if (this === $startTextNode[0]) {
                    CFICharacterOffset += characterOffset;
                    return false; // break
                }
                CFICharacterOffset += this.length;
            } else {
                CFICharacterOffset = 0; // resetting character offset
                CFIIndex += 2; // Incrementing to next potential CFI text position
            }
        });
        return "/" + CFIIndex + ":" + CFICharacterOffset;
    ...
    createCFIElementSteps : function ($currNode, topLevelElement, classBlacklist, elementBlacklist, idBlacklist) {
        ...
        if ($parentNode.is(topLevelElement) || $currNode.is(topLevelElement)) {
            ...
            if (topLevelElement === 'html') {
                ...
                if ($currNode.is(topLevelElement)) {
                    return '';
                } else {
                    return elementStep;
                }
